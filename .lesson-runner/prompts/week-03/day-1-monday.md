...
### Expected Outcome
By the end of this lesson, the student should have: **Store fetches products from a mock API with loading spinners and error states**.

### Acceptance Criteria
- You can explain today's focus in your own words: HTTP fundamentals, Promises, async/await, the fetch API.
- The expected outcome is present and reviewable: Store fetches products from a mock API with loading spinners and error states.
...
```text
I am stuck on today's focus: HTTP fundamentals, Promises, async/await, the fetch API. Ask me one diagnostic question at a time and help me find the smallest next step. Do not solve the whole lesson for me.
```
