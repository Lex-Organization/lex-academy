...
### Expected Outcome
By the end of this lesson, the student should have: **Multi-slice state architecture for the embroidery store: CartContext, FilterContext, UIContext, OrderContext**.

### Acceptance Criteria
- You can explain today's focus in your own words: Scaling state management with Context + useReducer — multiple contexts, context composition, avoiding the "god context" anti-pattern.
- The expected outcome is present and reviewable: Multi-slice state architecture for the embroidery store: CartContext, FilterContext, UIContext, OrderContext.
...
```text
I am stuck on today's focus: Scaling state management with Context + useReducer — multiple contexts, context composition, avoiding the "god context" anti-pattern. Ask me one diagnostic question at a time and help me find the smallest next step. Do not solve the whole lesson for me.
```
