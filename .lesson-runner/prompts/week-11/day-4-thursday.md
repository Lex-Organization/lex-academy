...
### Expected Outcome
By the end of this lesson, the student should have: **`/api/products`, `/api/cart`, `/api/orders` REST endpoints for the embroidery store**.

### Acceptance Criteria
- You can explain today's focus in your own words: Route Handlers — building REST API endpoints in Next.js with `route.ts`.
- The expected outcome is present and reviewable: `/api/products`, `/api/cart`, `/api/orders` REST endpoints for the embroidery store.
...
```text
I am stuck on today's focus: Route Handlers — building REST API endpoints in Next.js with `route.ts`. Ask me one diagnostic question at a time and help me find the smallest next step. Do not solve the whole lesson for me.
```
