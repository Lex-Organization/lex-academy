...
### Expected Outcome
By the end of this lesson, the student should have: **Add to cart, update quantity, remove from cart via Server Actions**.

### Acceptance Criteria
- You can explain today's focus in your own words: Server Actions — `"use server"`, form mutations, `revalidatePath`, `revalidateTag`, optimistic updates.
- The expected outcome is present and reviewable: Add to cart, update quantity, remove from cart via Server Actions.
...
```text
I am stuck on today's focus: Server Actions — `"use server"`, form mutations, `revalidatePath`, `revalidateTag`, optimistic updates. Ask me one diagnostic question at a time and help me find the smallest next step. Do not solve the whole lesson for me.
```
