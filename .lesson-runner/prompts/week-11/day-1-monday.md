...
### Expected Outcome
By the end of this lesson, the student should have: **Server-rendered product listing and detail pages fetching from data sources**.

### Acceptance Criteria
- You can explain today's focus in your own words: Data fetching in Server Components — async components, fetch with caching, parallel and sequential data fetching.
- The expected outcome is present and reviewable: Server-rendered product listing and detail pages fetching from data sources.
...
```text
I am stuck on today's focus: Data fetching in Server Components — async components, fetch with caching, parallel and sequential data fetching. Ask me one diagnostic question at a time and help me find the smallest next step. Do not solve the whole lesson for me.
```
